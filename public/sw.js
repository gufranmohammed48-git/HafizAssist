...
const FILES = [
  'index.html', 'mobile.html', 'src/entry.js', 'favicon.svg', 'src/app.js', 'src/alignment.js', 'src/debug.js', 'src/word-info.js',
  'src/offline.js', 'src/style.css', 'src/voice-search-worker.js', 'data/quran.json', 'data/HafsSmart.ttf',
  'data/mushaf-layout.json', 'data/tajweed-words.json', 'assets/bismillah.png',
...
self.addEventListener('fetch', event => {
  ...
  const isMobile = request.mode === 'navigate' && [url('mobile.html'), url('mobile')].includes(path);
  ...
  const key = isHome ? url('index.html') : isMobile ? url('mobile.html') : path;
  ...
  event.respondWith((async () => {
    ...
    try {
      ...
      if (saved?.ok) return isHome || isMobile ? navigationResponse(saved) : saved;
    ...
    return isHome || isMobile ? navigationResponse(response) : response;
  })());
});
