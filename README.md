...
## Continuous recitation and word help (V11)

Completing a page turns to the next page without restarting the microphone, worker stream, timer, or cumulative counts. Already-recognized trailing phonemes continue into the next page, including across chapter openings. Page 604 wraps to page 1. Stopping manually, hiding the tab, microphone loss, and processing errors still stop capture. A session summary covers the pages traversed. A saved transcript boundary prevents a revised hypothesis from replaying the previous page into the next page.
